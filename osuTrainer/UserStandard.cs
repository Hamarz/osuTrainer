...
{
    internal class UserStandard : IUser
    {
        ...
        /// </summary>
        public UserStandard(string username)
        {
            using (var client = new CustomWebClient())
            {
                //standard
                string json = client.DownloadString(GlobalVars.UserAPI + username + GlobalVars.Mode + 0);
                ...
                json = client.DownloadString(GlobalVars.UserBestAPI + User_id + GlobalVars.Mode + 0);
                BestScores = JsonSerializer.DeserializeFromString<List<UserBest>>(json);
            }
        }

        public UserStandard(string json, bool isjson = true)
        {
            using (var client = new CustomWebClient())
            {
                Match match = Regex.Match(json, @"""user_id"":""(.+?)"".+?""username"":""(.+?)"".+?""pp_rank"":""(.+?)"".+?""level"":""(.+?)"".+?""pp_raw"":""(.+?)"".+?""country"":""(.+?)""");
        ...
        {
            return client.DownloadString(GlobalVars.UserAPI + username + GlobalVars.Mode + 0);
        }
    }
}